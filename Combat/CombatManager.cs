...
{
    public sealed class CombatManager                                        //клсс боя     
    {
        private readonly Random _random = new();
        
        public Unit StartCombat(Unit player, Unit enemy) => PlayCombatRoutine(player, enemy);

        private Unit PlayCombatRoutine(Unit player, Unit enemy)                    //запуск всей рутины боёвки 
        {
            Console.WriteLine(GetCombatString());  //получение информации о текущем сосотоянии боя 
            while (player.Health > 0 && enemy.Health > 0) // пока здоровье больше нуля бой идет 
            {
        ...
        private void HandleCombatInput(Unit player, Unit enemy, RockPaperScissors rockPaperScissors)          //игра в камень ножницы бумага
