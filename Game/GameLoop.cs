...
namespace GamePrototype.Game
{
    public sealed class GameLoop
    {
        ...
        {
            var currentRoom = _dungeon;                       //есть небольшие отличия
        ...
        {
            success = true;
            if (currentRoom.Loot != null)  //если у нас есть лут то он автоматически добавляется
            {
                _player.AddItemToInventory(currentRoom.Loot);
            }
            if (currentRoom.Enemy != null)  //если есть враг, начинаем бой 
            {
                if (_combatManager.StartCombat(_player, currentRoom.Enemy) == _player)
                {
                    _player.HandleCombatComplete();    //если игрок, двигаемся дальше если нет
